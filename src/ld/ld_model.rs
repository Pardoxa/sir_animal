    ...
    fn iterate_once(&mut self)
    {
        ...
        for &index in self.new_infections_list_dogs.iter()
        {
            ...
            if container.contained().get_infectiouse_neighbor_count() == 1 {
                ...
                'scope: loop {
                    ...
                    for &idx in neighbors
                    {
                        ...
                        if node.is_infected()
                        {
                            ...
                            let dog_lambda = node_to_transition.get_gamma_trans().trans_animal;
                            //println!("dog: {dog_lambda} {index}");
                            break 'scope;
                        }
                    }
                ...
                'outer: loop{
                    let mut iter = self.dual_graph.graph_1_mut().contained_iter_neighbors_mut(index);
